  ...
  useEffect(() => {
    ...
    if (selectedAdmin) {
      if (selectedAdmin.id === "adm1") {
        ...
        setPagePermission(selectedAdmin.permissions.products || null);
      }
    }
  }, []);
...
const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#F9FAFB] text-[#1F2937] font-sans overflow-x-hidden">
      ...
      <LeftSidebar activePage="products" isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} />
        ...
        <div className="space-y-6">
          ...
          {paginatedProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center bg-white rounded-xl border border-gray-150 p-12 shadow-sm text-center">
              ...
              />
              <h3 className="text-lg font-black text-stark-text mb-2">No Products Found</h3>
                        ...
                        <tr key={product.id} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 mt-1 text-[10px]">
                              <span className="font-bold text-stark-primary">${product.price ?? 0}</span>
                              <span className="text-gray-300">•</span>
                              <span className="text-stark-muted">ID: {product.id}</span>
                            </div>
                          </td>
                            ...
                            {resolvedVendor ? (
                              <div className="flex items-center space-x-2">
                                <div className="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center font-bold text-gray-600 text-[10px] uppercase">
                                  {resolvedVendor.name[0]}
                                </div>
                                <span className="font-medium text-stark-text">{resolvedVendor.name}</span>
                              </div>
                            ) : (
                              <span className="text-stark-muted font-medium">Unknown Vendor</span>
                            )}
                          </td>
                          ...
                          <td className="px-5 py-3">
                            <div className="flex flex-col gap-1 items-start">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${stockBadgeClass}`}>
                                {stockLabel}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3">
                            <button
                              ...
                              className={`px-2.5 py-0.5 rounded-full border text-[10px] font-black cursor-pointer ${activeBadgeClass} ${pagePermission === "view" ? "opacity-70 cursor-not-allowed bg-gray-50 text-gray-400 border-gray-200" : "cursor-pointer hover:scale-105"}`}
                            >
                              {product.status ?? "Active"}
                            </button>
                          </td>

...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    ...
    if (selectedAdmin) {
      if (selectedAdmin.id === "adm1") {
        ...
        setPagePermission(selectedAdmin.permissions.categories || null);
      }
    }
  }, []);
  ...
  const [pendingStatusCategory, setPendingStatusCategory] = useState<MainCategory | null>(null);
  ...
  const [pendingDeleteCategory, setPendingDeleteCategory] = useState<MainCategory | null>(null);
  ...
  // Metrics Metrics derived from state
  const metrics = useMemo(() => {
    const total = categories.length;
    const active = categories.filter((c) => c.status === "Active").length;
    const inactive = categories.filter((c) => c.status === "Inactive").length;
    const totalSubs = categories.reduce((sum, c) => sum + (c.subCategoryCount || 0), 0);
  ...
  const handleOpenEditModal = (cat: MainCategory) => {
    setEditingCategory(cat);
    setFormName(cat.name);
    setFormIconKey(cat.iconName);
  ...
  const handleSaveCategory = (e: React.FormEvent) => {
    ...
    // Constraint validation: Unique name case-insensitive
    const duplicate = categories.some(
      (c) => c.name.toLowerCase() === cleanName.toLowerCase() && (!editingCategory || c.id !== editingCategory.id)
    );
    if (duplicate) {
      ...
      setCategories(
        categories.map((c) =>
          c.id === editingCategory.id
            ? { ...c, name: cleanName, iconName: formIconKey, status: formStatus }
            : c
        )
      );
    } else {
      // Add
      const maxSortOrder = categories.reduce((max, c) => (c.sortOrder > max ? c.sortOrder : max), 0);
  ...
  // Toggle Status Click
  const handleToggleStatusClick = (cat: MainCategory) => {
    if (cat.status === "Active") {
      // Activating -> Inactive: Warning about nested subcategories inactivation
      setPendingStatusCategory(cat);
      setIsStatusWarningOpen(true);
    } else {
      // Inactive -> Active: Toggle instantly
      setCategories(
        categories.map((c) => (c.id === cat.id ? { ...c, status: "Active" } : c))
      );
    }
  };
  ...
  const confirmInactivation = () => {
    if (pendingStatusCategory) {
      setCategories(
        categories.map((c) =>
          c.id === pendingStatusCategory.id ? { ...c, status: "Inactive" } : c
        )
      );
    }
  ...
  // Delete Click
  const handleDeleteClick = (cat: MainCategory) => {
    if (cat.subCategoryCount > 0) {
      // Block deletion
      setBlockedItemName(cat.name);
  ...
  const confirmDelete = () => {
    if (pendingDeleteCategory) {
      setCategories(categories.filter((c) => c.id !== pendingDeleteCategory.id));
    }
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#F9FAFB] text-[#1F2937] font-sans overflow-x-hidden">
      <LeftSidebar activePage="categories" isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} />
      ...
      {pagePermission === null ? (
        ...
        <div className="flex md:hidden items-center justify-between bg-white border border-gray-200 p-4 rounded-xl mb-6 shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-bold text-sm">Stark Categories</span>
          </div>
          <button 
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8" data-purpose="page-header">
          <div>
            ...
            <p className="text-sm text-stark-muted">
              Configure and organize Main Categories for your marketplace.
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-stark-muted tracking-wider">Total Categories</p>
              <h4 className="text-xl font-black text-stark-text mt-1">{metrics.total}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-green-600 tracking-wider">Active Categories</p>
              <h4 className="text-xl font-black text-green-700 mt-1">{metrics.active}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Inactive Categories</p>
              <h4 className="text-xl font-black text-gray-600 mt-1">{metrics.inactive}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-stark-primary tracking-wider">Total Sub-Categories</p>
              <h4 className="text-xl font-black text-stark-primary mt-1">{metrics.totalSubs}</h4>
            </div>
          </div>
          ...
          {filteredAndSortedCategories.length === 0 ? (
            <div className="flex flex-col items-center justify-center bg-white rounded-xl border border-gray-150 p-12 shadow-sm text-center">
              ...
              />
              <h3 className="text-lg font-black text-stark-text mb-2">No Categories Found</h3>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-150">
                      ...
                      <th className="px-5 py-4 text-[10px] font-black text-stark-muted uppercase tracking-wider">Category Details</th>
                      <th className="px-5 py-4 text-[10px] font-black text-stark-muted uppercase tracking-wider hidden sm:table-cell">Sub-Categories Nested</th>
                        ...
                        <tr key={cat.id} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <div className="w-9 h-9 bg-stark-primary/10 border border-stark-primary/20 rounded-lg flex items-center justify-center text-stark-primary">
                              {renderCategoryIcon(cat.iconName, "w-4.5 h-4.5")}
                            </div>
                          </td>

                          <td className="px-5 py-3">
                            <div className="font-bold text-stark-text">{cat.name}</div>
                            <div className="text-[10px] text-stark-muted mt-0.5">ID: {cat.id}</div>
                          </td>
                            ...
                            <div className="flex justify-end gap-1.5">
                              <button
                                ...
                                className={`p-1 rounded transition-all ${
                                  pagePermission === "view"
                                    ? "opacity-30 cursor-not-allowed text-stark-muted"
                                    : "hover:bg-gray-100 text-stark-muted hover:text-stark-primary cursor-pointer"
                                }`}
                                title="Edit"
                              >
                              ...
                              <button
                                ...
                                className={`p-1 rounded transition-all ${
                                  pagePermission !== "create"
                                    ? "opacity-30 cursor-not-allowed text-stark-muted"
                                    : "hover:bg-red-50 text-stark-muted hover:text-red-650 cursor-pointer"
                                }`}
                                title="Delete"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            ...
            <div className="p-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-stark-muted">
                  Configure details for your top-level product category node.
                </p>
              </div>
              <button 
                type="button"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-stark-muted">Category Name*</label>
                <input
                  type="text"
                  required
                  value={formName}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-stark-muted">Category Icon*</label>
      ...
      {isStatusWarningOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <h3 className="text-base font-black text-stark-text mb-2">Inactivate Category?</h3>
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Inactivating this Main Category will automatically inactivate all of its nested Sub Categories. Do you wish to proceed?
            </p>
      ...
      {isDeleteBlockerOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Cannot delete Main Category &quot;{blockedItemName}&quot; because it contains child Sub Categories. Please delete or reassign all sub-categories first.
            </p>
      ...
      {isConfirmDeleteOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <h3 className="text-base font-black text-stark-text mb-2">Delete Category?</h3>
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Are you sure you want to permanently delete the Main Category &quot;{pendingDeleteCategory?.name}&quot;? This action cannot be undone.
            </p>

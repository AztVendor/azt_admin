...
export default function MiniCategoriesPage() {
  ...
  useEffect(() => {
    ...
    if (selectedAdmin) {
      if (selectedAdmin.id === "adm1") {
        ...
        setPagePermission(selectedAdmin.permissions.minicategories || null);
      }
    }
  }, []);
...
const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  ...
  const [miniCategories, setMiniCategories] = useState<MiniCategory[]>(INITIAL_MINI_CATEGORIES);
  ...
  const [editingMini, setEditingMini] = useState<MiniCategory | null>(null);
  ...
  const [pendingStatusMini, setPendingStatusMini] = useState<MiniCategory | null>(null);

  const [isParentInactiveBlockerOpen, setIsParentInactiveBlockerOpen] = useState(false);
  const [blockedParentType, setBlockedParentType] = useState<"main" | "sub">("sub");
  ...
  const [pendingDeleteMini, setPendingDeleteMini] = useState<MiniCategory | null>(null);

  // Sorting & Filtering Helper
  const filteredAndSortedMiniCategories = useMemo(() => {
    return [...miniCategories]
      .filter((mini) => {
        const matchesSearch = mini.name.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesStatus = statusFilter === "all" || mini.status === statusFilter;
        return matchesSearch && matchesStatus;
      })
      .sort((a, b) => {
        // Sort alphabetically or naturally by Name
        return a.name.localeCompare(b.name);
      });
  }, [miniCategories, searchQuery, statusFilter]);

  // Metrics derived from state
  const metrics = useMemo(() => {
    const total = miniCategories.length;
    const active = miniCategories.filter((m) => m.status === "Active").length;
    const inactive = miniCategories.filter((m) => m.status === "Inactive").length;
    const totalChildren = miniCategories.reduce((sum, m) => sum + (m.childCount || 0), 0);
  ...
  // Handle Parent Main selection change to dynamically adjust parent Sub list
  const handleMainChangeInForm = (mainId: string) => {
    setFormMainId(mainId);
    const firstSubOfMain = subCategories.find((s) => s.parentId === mainId);
    setFormSubId(firstSubOfMain ? firstSubOfMain.id : "");
  };
  ...
  const handleOpenAddModal = () => {
    ...
    const initialSub = subCategories.find((s) => s.parentId === initialMainId);
    setFormSubId(initialSub ? initialSub.id : "");
  ...
  const handleOpenEditModal = (mini: MiniCategory) => {
    setEditingMini(mini);
    setFormName(mini.name);
    
    // Look up parent Sub Category to get parent Main Category
    const parentSub = subCategories.find((s) => s.id === mini.parentId);
    const mainId = parentSub ? parentSub.parentId : "";
    setFormMainId(mainId);
    setFormSubId(mini.parentId);
    setFormIconKey(mini.iconName);
    setFormStatus(mini.status);
    setFormError("");
    setIsAddEditModalOpen(true);
  };

  // Form Submit Handler
  const handleSaveMiniCategory = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanName = formName.trim();
    if (!cleanName) {
      setFormError("Mini Category Name is required.");
      return;
    }

    if (!formSubId) {
      setFormError("Parent Sub Category is required.");
      return;
    }

    // Validation: name is unique within selected parent Sub Category
    const duplicate = miniCategories.some(
      (m) =>
        m.parentId === formSubId &&
        m.name.toLowerCase() === cleanName.toLowerCase() &&
        (!editingMini || m.id !== editingMini.id)
    );
    if (duplicate) {
      const subName = subCategories.find((s) => s.id === formSubId)?.name || "Sub Category";
      setFormError(`A Mini Category named "${cleanName}" already exists under "${subName}".`);
      return;
    }

    // Validation: status cannot be Active if parent Sub Category OR parent Main Category is Inactive
    const parentSub = subCategories.find((s) => s.id === formSubId);
    const parentMain = mainCategories.find((c) => c.id === (parentSub ? parentSub.parentId : ""));

    if (formStatus === "Active") {
      if (parentSub && parentSub.status === "Inactive") {
        setFormError(`Cannot set Mini Category to Active because parent Sub Category "${parentSub.name}" is Inactive.`);
        return;
      }
      if (parentMain && parentMain.status === "Inactive") {
        setFormError(`Cannot set Mini Category to Active because parent Main Category "${parentMain.name}" is Inactive.`);
        return;
      }
    }
    ...
    if (editingMini) {
      ...
      setMiniCategories(
        miniCategories.map((m) =>
          m.id === editingMini.id
            ? { ...m, name: cleanName, parentId: formSubId, iconName: formIconKey, status: formStatus }
            : m
        )
      );
    } else {
      // Add
      const newMini: MiniCategory = {
        id: `mn_${Date.now()}`,
        parentId: formSubId,
        name: cleanName,
      ...
      setMiniCategories([...miniCategories, newMini]);
    }
  ...
  // Toggle Status Click
  const handleToggleStatusClick = (mini: MiniCategory) => {
    if (mini.status === "Active") {
      // Active -> Inactive: Warn deactivation cascades to macro children
      setPendingStatusMini(mini);
      ...
      if (parentSub && parentSub.status === "Inactive") {
        ...
        setMiniCategories(
          miniCategories.map((m) => (m.id === mini.id ? { ...m, status: "Active" } : m))
        );
      }
    }
  };
  ...
  const confirmInactivation = () => {
    if (pendingStatusMini) {
      setMiniCategories(
        miniCategories.map((m) =>
          m.id === pendingStatusMini.id ? { ...m, status: "Inactive" } : m
        )
      );
    }
    setIsStatusWarningOpen(false);
    setPendingStatusMini(null);
  };

  // Delete Click
  const handleDeleteClick = (mini: MiniCategory) => {
    if (mini.childCount > 0) {
      setBlockedItemName(mini.name);
      setIsDeleteBlockerOpen(true);
    } else {
      setPendingDeleteMini(mini);
      setIsConfirmDeleteOpen(true);
    }
  };

  const confirmDelete = () => {
    if (pendingDeleteMini) {
      setMiniCategories(miniCategories.filter((m) => m.id !== pendingDeleteMini.id));
    }
    setIsConfirmDeleteOpen(false);
    setPendingDeleteMini(null);
  };

  return (
    <div className="flex min-h-screen w-full bg-[#F9FAFB] text-[#1F2937] font-sans overflow-x-hidden">
      
      <LeftSidebar activePage="minicategories" isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} />
  ...
  /* BEGIN: MainContent */
      <main className="ml-0 md:ml-64 flex-1 min-w-0 w-full p-4 md:p-8 min-h-screen transition-all duration-300">
        ...
        <div className="flex md:hidden items-center justify-between bg-white border border-gray-200 p-4 rounded-xl mb-6 shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-bold text-sm">Stark Mini Categories</span>
          </div>
          <button 
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8" data-purpose="page-header">
          <div>
            <h2 className="text-2xl font-bold">Mini Category Management</h2>
            <p className="text-sm text-stark-muted">
              Configure and organize Mini Categories nested under Sub Categories.
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-stark-muted tracking-wider">Total Mini Categories</p>
              <h4 className="text-xl font-black text-stark-text mt-1">{metrics.total}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-green-600 tracking-wider">Active Mini Categories</p>
              <h4 className="text-xl font-black text-green-700 mt-1">{metrics.active}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Inactive Mini Categories</p>
              <h4 className="text-xl font-black text-gray-600 mt-1">{metrics.inactive}</h4>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-150 shadow-xs">
              <p className="text-[10px] uppercase font-bold text-stark-primary tracking-wider">Total Macro Children</p>
          ...
          <section className="bg-white rounded-xl border border-gray-150 p-4 shadow-sm flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search mini categories..."
                value={searchQuery}
          ...
          {/* Mini Categories Table View */}
          {filteredAndSortedMiniCategories.length === 0 ? (
            <div className="flex flex-col items-center justify-center bg-white rounded-xl border border-gray-150 p-12 shadow-sm text-center">
              ...
              />
              <h3 className="text-lg font-black text-stark-text mb-2">No Mini Categories Found</h3>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-150">
                      ...
                      <th className="px-5 py-4 text-[10px] font-black text-stark-muted uppercase tracking-wider">Mini Category Details</th>
                      ...
                      <th className="px-5 py-4 text-[10px] font-black text-stark-muted uppercase tracking-wider hidden sm:table-cell">Macro Children</th>
                  ...
                  <tbody className="divide-y divide-gray-100 text-xs">
                    {filteredAndSortedMiniCategories.map((mini) => {
                      ...
                      return (
                        <tr key={mini.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-5 py-3">
                            <div className="w-9 h-9 bg-stark-primary/10 border border-stark-primary/20 rounded-lg flex items-center justify-center text-stark-primary">
                              {renderCategoryIcon(mini.iconName, "w-4.5 h-4.5")}
                            </div>
                          </td>

                          <td className="px-5 py-3">
                            <div className="font-bold text-stark-text">{mini.name}</div>
                            <div className="text-[10px] text-stark-muted mt-0.5">ID: {mini.id}</div>
                          </td>
                          ...
                          <td className="px-5 py-3">
                            <button
                              onClick={() => handleToggleStatusClick(mini)}
                          ...
                          <td className="px-5 py-3 text-right">
                            <div className="flex justify-end gap-1.5">
                              <button
                                onClick={() => handleOpenEditModal(mini)}
                                disabled={pagePermission === "view"}
                              ...
                              <button
                                onClick={() => handleDeleteClick(mini)}
            ...
            <div className="p-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <div>
                <h3 className="font-black text-lg text-stark-text">
                  {editingMini ? "Edit Mini Category" : "Add Mini Category"}
                </h3>
                <p className="text-xs text-stark-muted">
                  Configure details for your three-level nested product category node.
                </p>
              </div>
              <button 
                type="button"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[10px] text-stark-muted">
                  Nests under both the selected Main and Sub categories.
                </p>
              </div>

              {/* Name Input */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-stark-muted">Mini Category Name*</label>
                <input
                  type="text"
                  required
                  value={formName}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-stark-muted">Mini Category Icon*</label>
      ...
      {/* END: Add/Edit Mini Category Modal */}
      ...
      {isStatusWarningOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <h3 className="text-base font-black text-stark-text mb-2">Inactivate Mini Category?</h3>
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Inactivating this Mini Category will automatically inactivate all nested Macro categories under it. Do you wish to proceed?
            </p>
      ...
      {isParentInactiveBlockerOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Cannot activate Mini Category because parent {blockedParentType === "main" ? "Main Category" : "Sub Category"} &quot;{blockedParentName}&quot; is Inactive. Please activate the parent first.
            </p>
      ...
      {isDeleteBlockerOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Cannot delete Mini Category &quot;{blockedItemName}&quot; because it contains nested Macro categories. Please delete or reassign them first.
            </p>
      ...
      {isConfirmDeleteOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full relative z-10 p-6 flex flex-col shadow-2xl border border-gray-100 text-center animate-fade-in">
            ...
            <h3 className="text-base font-black text-stark-text mb-2">Delete Mini Category?</h3>
            <p className="text-xs text-stark-muted leading-relaxed mb-6">
              Are you sure you want to permanently delete the Mini Category &quot;{pendingDeleteMini?.name}&quot;? This action cannot be undone.
            </p>
